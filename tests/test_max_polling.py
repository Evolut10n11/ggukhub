...
class _OperatorServiceStub:
    def __init__(self) -> None:
        self.messages: list[tuple[int, int, str]] = []
        self.callbacks: list[tuple[int, int, str]] = []
        self.operator_ids = {909}
    ...
    async def handle_operator_callback(self, chat_id: int, user_id: int, payload: str) -> bool:
        if not self.is_operator(user_id):
            return False
        self.callbacks.append((chat_id, user_id, payload))
        return payload.startswith("op_")
...
@pytest.mark.asyncio
async def test_max_operator_message_is_routed_before_dialog_flow() -> None:
    settings = Settings(telegram_bot_token="test-token", max_bot_token="max-token")
    registry = BuildingRegistry(
        complexes=[ComplexInfo(name="Complex 1")],
        standalone_houses=[],
    )
    services = SimpleNamespace(building_registry=registry, max_operator_service=_OperatorServiceStub())
    poller = MaxPolling(settings, services)  # type: ignore[arg-type]
    poller._dialog_service = _DialogServiceStub()  # type: ignore[assignment]
    poller._client = _MaxClientStub()  # type: ignore[assignment]

    await poller._handle_message(
        {
            "message": {
                "sender": {"user_id": 909, "name": "Operator"},
                "recipient": {"chat_id": 303},
                "body": {"text": "/queue", "attachments": []},
            }
        }
    )

    assert services.max_operator_service.messages == [(303, 909, "/queue")]
    assert poller._dialog_service.calls == []  # type: ignore[attr-defined]

@pytest.mark.asyncio
async def test_max_id_command_returns_current_user_id_before_operator_routing() -> None:
    settings = Settings(telegram_bot_token="test-token", max_bot_token="max-token")
    registry = BuildingRegistry(
        complexes=[ComplexInfo(name="Complex 1")],
        standalone_houses=[],
    ...
    await poller._handle_message(
        {
            "message": {
                "sender": {"user_id": 909, "name": "Operator"},
                "recipient": {"chat_id": 303},
                "body": {"text": "/id", "attachments": []},
            }
        }
    )

    assert services.max_operator_service.messages == []
    assert poller._dialog_service.calls == []  # type: ignore[attr-defined]
    assert poller._client.sent_messages[-1]["text"] == "Ваш MAX user_id: `909`"  # type: ignore[attr-defined]
