...
class _NotifierStub:
    ...
    async def send_user_message(self, user: User, text: str) -> bool:
        self.messages.append((user.platform_user_id, text))
        return True
...
class _BitrixCreateStub:
    ...
    async def find_contact_by_phone(self, phone: str) -> str | None:
        self.find_contact_calls.append(phone)
        return None
...
async def _build_services(db_path: Path, *, bitrix_client: Any, notifier: _NotifierStub) -> tuple[AppServices, Any]:
    ...
    services = AppServices(
        ...
        notifier=notifier,
        building_registry=BuildingRegistry.from_file(Path("data/housing_complexes.json")),
...
async def test_bitrix_client_create_ticket_builds_expected_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "Лифт не работает" in str(fields[settings.bitrix_field_title])
...
async def test_dialog_flow_creates_report_and_persists_bitrix_id(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert call["jk"] == "Прайд Парк"
        assert call["address"] == "дом 5, подъезд 3, кв 78"
        assert call["category"] == "accident"
        ...
        assert any("B24-7001" in text for text in replies)
...
async def test_dialog_flow_bitrix_failure_keeps_local_report_and_logs_failure(tmp_path: Path) -> None:
    ...
    try:
        replies = await _run_full_dialog_flow(services, user_id=300_002, phone="+79990002233")
        await services.dialog_runtime.wait_background_tasks()
        ...
        assert any("Не удалось передать автоматически" in text for text in replies)
        assert not notifier.messages
    finally:
        await engine.dispose()
...
async def test_report_correction_step_allows_fixing_category_before_creation(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert report is not None
        assert report.category == "accident"
    finally:
        await engine.dispose()
